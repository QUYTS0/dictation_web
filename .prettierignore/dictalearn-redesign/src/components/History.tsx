  ...
  return (
    <div className="flex-1 flex flex-col font-sans w-full relative z-10 h-full overflow-hidden">
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 py-8 flex flex-col gap-8 overflow-y-auto">
        ...
        {/* Header Section */}
        <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/40 dark:border-white/10 pb-6 text-slate-900 dark:text-white">
          <div>
             <h1 className="text-3xl font-bold tracking-tight mb-2">Practice History</h1>
             <p className="text-slate-500 dark:text-slate-400 text-sm">Track your dictation sessions and progress.</p>
          </div>
          <div className="flex gap-4 w-full md:w-auto">
             <div className="bg-white/50 dark:bg-slate-800/40 backdrop-blur-md p-3 px-5 rounded-2xl border border-white/60 dark:border-white/10 shadow-sm flex items-center gap-3 flex-1 md:flex-initial">
               ...
               <div>
                 ...
                 <p className="text-lg font-black leading-none">2h 15m</p>
               </div>
             </div>
             <div className="bg-white/50 dark:bg-slate-800/40 backdrop-blur-md p-3 px-5 rounded-2xl border border-white/60 dark:border-white/10 shadow-sm flex items-center gap-3 flex-1 md:flex-initial">
        ...
        <section className="flex flex-col gap-6 pb-12">
           {historyItems.map((item, idx) => (
             <motion.div 
               ...
               className="group relative bg-white/40 dark:bg-slate-800/40 backdrop-blur-xl rounded-[2rem] border border-white/60 dark:border-white/10 shadow-lg p-5 flex flex-col sm:flex-row gap-6 hover:-translate-y-1 transition-all cursor-pointer"
             >
               {/* Thumbnail Area */}
               <div className="relative w-full sm:w-56 aspect-[16/9] rounded-2xl overflow-hidden bg-slate-800 shrink-0 shadow-md">
                 ...
                 {item.progress === 100 && (
                   <div className="absolute top-2 right-2 bg-emerald-500/90 text-white backdrop-blur px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm flex items-center gap-1">
               ...
               <div className="flex-1 flex flex-col justify-between py-1">
                 <div>
                   <div className="flex justify-between items-start gap-4 mb-2">
                     <h3 className="font-bold text-xl text-slate-900 dark:text-white group-hover:text-indigo-600 transition-colors leading-tight tracking-tight">{item.title}</h3>
                   </div>
                   <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-4 tracking-tight">{item.channel}</p>
                 </div>
                 
                 <div>
                   {/* Metrics Row */}
                   <div className="flex flex-wrap gap-3 mb-5">
                      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 bg-white/40 dark:bg-white/5 px-3 py-1.5 rounded-xl border border-white/40 dark:border-white/10 shadow-sm shadow-black/5">
                      ...
                      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-xl border border-emerald-500/20 shadow-sm shadow-black/5">
                   ...
                   {/* Progress */}
                   <div>
                     <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 leading-none">
                       <span>Session Progress</span>
                       <span>{item.completedSents} / {item.totalSents} Sentences</span>
                     </div>
                     <div className="relative w-full bg-slate-200/50 dark:bg-slate-700/50 border border-white/40 dark:border-white/5 rounded-full h-2 overflow-hidden shadow-inner flex">
                       <div className="bg-indigo-500 h-full rounded-full transition-all duration-1000 shadow-[0_0_8px_rgba(79,70,229,0.3)]" style={{ width: `${item.progress}%` }}></div>
                     </div>
                   </div>
                 </div>
               </div>
