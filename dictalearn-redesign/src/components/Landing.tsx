    ...
    <div className="flex-1 flex flex-col font-sans w-full">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-16 w-full max-w-7xl mx-auto">
        ...
        <section className="text-center max-w-3xl mx-auto mb-16 pt-8">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary-50 text-primary-700 text-xs font-semibold mb-6 border border-primary-100">
              <Zap size={14} className="text-primary-500" />
              <span>Master English through listening</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-semibold tracking-tight text-slate-900 mb-6 text-balance leading-tight">
          ...
          <motion.div 
            ...
            className="bg-white/40 backdrop-blur-xl p-3 md:p-4 rounded-3xl shadow-xl border border-white/60 flex flex-col sm:flex-row gap-3 max-w-2xl mx-auto focus-within:ring-2 focus-within:ring-primary-500/30 transition-all"
          >
            <div className="relative flex-1 flex items-center">
              ...
              <input 
                ...
                className="w-full bg-transparent border-none focus:ring-0 pl-12 pr-4 py-3 text-slate-900 placeholder:text-slate-400 outline-none text-base"
              />
            </div>
            <button 
              onClick={() => onNavigate('workspace')}
              className="bg-primary-600 hover:bg-primary-700 text-white px-8 py-3 rounded-xl font-medium flex items-center justify-center gap-2 transition-colors shadow-sm whitespace-nowrap"
            >
              Start Dictation <ArrowRight size={18} />
            </button>
          </motion.div>
          <p className="text-sm text-slate-500 mt-4">Start without signing in. Sign in later to save progress.</p>
        </section>
...
function FeatureCard({ icon, title, description, delay }: { icon: React.ReactNode, title: string, description: string, delay: number }) {
  return (
    <motion.div 
      ...
      className="bg-white/40 backdrop-blur-xl p-6 rounded-3xl border border-white/60 shadow-xl hover:-translate-y-1 transition-all"
    >
      <div className="w-12 h-12 rounded-xl bg-white/60 border border-white/80 flex items-center justify-center text-indigo-600 mb-4 shadow-sm">
        {icon}
      </div>
